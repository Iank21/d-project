...
export default async function TestCardAdmin() {
  ...
  return(
      <div className="flex gap-4 flex-col">
        {tests?.map((test: Test) => (
          ...
          <Link href='#' className="cursor-pointer w-fit flex items-center gap-5 self-start rounded-lg bg-blue-500 px-6 py-3 text-sm font-medium text-white transition-colors hover:bg-blue-400 md:text-base">Изменить</Link>
        </div>
        ))}
      </div>
  );
